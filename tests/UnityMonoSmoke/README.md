...
whose `ref` and `out` values flow through an original-call delegate. An `in`
parameter and a writable managed by-reference return are verified through their
original-call delegates as well. Virtual base and override implementations are
also hooked as independent targets. It is not a sample game or a compatibility
claim.
